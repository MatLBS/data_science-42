...
def clustering(cur):
    ...
    gold_mean_purchase = np.mean([x[5] for x in data if x[3] == 'gold'])
    ...
    platinium_mean_purchase = np.mean([x[5] for x in data if x[3] == 'platinium'])
    ...
    inactive_mean_purchase = np.mean([x[5] for x in data if x[3] == 'inactive'])
    ...
    new_mean_purchase = np.mean([x[5] for x in data if x[3] == 'new'])

    y = [gold_median_freq, platinium_median_freq, new_median_freq, inactive_median_freq]
    x = [gold_median_rec, platinium_median_rec, new_median_rec, inactive_median_rec]
    name = ["Gold", "Platinium", "New", "Inactive"]
    mean = [gold_mean_purchase, platinium_mean_purchase, new_mean_purchase, inactive_mean_purchase]
    sizes = [int(gold_median_freq), int(platinium_median_freq), int(new_median_freq), int(inactive_median_freq)]

    fig, ax = plt.subplots()

    for i in range(len(x)):
        ax.plot(x[i], y[i], marker='o', markersize=sizes[i], color=colors[i])
        plt.text(float(x[i]), float(y[i]) + 1.5, f"Average \"{name[i]} customers\": {mean[i]:.2f} ₳",)
